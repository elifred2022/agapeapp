...
const DescuentoEfectivo = ({ arregloAlmacenPorcentajeEfectivo, almacenPorcentEfectivo }) => {
  const [descuento, setDescuento] = useState('');
  const [inputDisabled, setInputDisabled] = useState(false)

  const traerPorcentajeEfectivo = almacenPorcentEfectivo.reduce( (acc, elem) => acc = parseInt(elem.descuento), 0 );
  
  useEffect(() => {  // funcion para desactivar el input
...
export default DescuentoEfectivo;
